...
class Enemy:
    ...
    def chase_player(self, player_pos: Tuple[int, int], speed: int = 2):
        px, py = player_pos
        ...
        if dist < 100:
            ...
            if dx != 0 or dy != 0:
                norm = (dx ** 2 + dy ** 2) ** 0.5
                self.dx = int(speed * dx / norm)
                self.dy = int(speed * dy / norm)
        else:
...
class Game:
    ...
    def load_sounds(self):
        try:
            self.snd_attack = pygame.mixer.Sound("attack.wav")
            self.snd_enemy_die = pygame.mixer.Sound("enemy_die.wav")
            self.snd_hit = pygame.mixer.Sound("hit.wav")
        except Exception:
            self.snd_attack = self.snd_enemy_die = self.snd_hit = None
    ...
    def load_sprites(self):
        try:
            self.player_spritesheet = pygame.image.load("player.png").convert_alpha()
    ...
    def all_enemies_dead_on_floor(self) -> bool:
        return all(room.alive_enemy_count == 0 for room in self.all_rooms.values() if room.floor == self.current_floor)
    ...
    def try_attack(self):
        ...
        if self.snd_attack: self.snd_attack.play()
        ...
        for enemy in self.current_room.enemies:
            if enemy.is_alive and attack_box.colliderect(enemy.rect):
                if enemy.take_damage(self.current_damage):
                    ...
                    if self.snd_enemy_die: self.snd_enemy_die.play()
    ...
    def check_collisions(self, keys: pygame.key.ScancodeWrapper):
        ...
        for door in self.current_room.doors:
            if self.player_rect.colliderect(door.rect):
                ...
                if (target_room.room_type in [RoomType.STAIRS_UP, RoomType.STAIRS_DOWN]) and not self.all_enemies_dead_on_floor():
                    lock_msg = self.small_font.render("Defeat all enemies on this floor to use stairs!", True, Color.RED)
                    self.screen.blit(lock_msg, (Config.SCREEN_WIDTH//2 - lock_msg.get_width()//2, Config.SCREEN_HEIGHT - 70))
                    continue
                if keys[pygame.K_e]:
        ...
        if self.current_room.room_type == RoomType.STAIRS_UP and self.current_room.cleared:
            if not self.all_enemies_dead_on_floor():
                pass
    ...
    def update(self):
        for enemy in self.current_room.enemies:
            if enemy.is_alive:
                enemy.chase_player(self.player_rect.center)
    ...
    def draw(self):
        ...
        for door in self.current_room.doors:
            ...
            if door.label:
                ...
                self.screen.blit(label, (draw_rect.centerx - label.get_width()//2,draw_rect.centery - label.get_height()//2))
        if self.current_room.room_type == RoomType.STAIRS_UP:
            if self.current_room.cleared:
                if not self.all_enemies_dead_on_floor():
                    msg = self.ui_font.render("⬆️ Defeat all enemies on this floor!", True, Color.RED)
                else:
        ...
        self.draw_hud()
        if pygame.time.get_ticks() < self.hit_effect_until:
