...
# --- Player setup ---
player_rect = pygame.Rect(WIDTH//2-20, HEIGHT//2-12, 20, 30)
player_speed = 5
player_dir = "down"
max_hp = 5
hp = max_hp
...
# Load LPC sprite sheet
# Replace "player.png" with the path to your downloaded sheet
# Adjust frame_width/frame_height to match your sheet frames
...
def make_enemies(amount, speed):
    ...
    for _ in range(amount):
        x = random.randint(50, ROOM_WIDTH - 90)
        y = random.randint(50, ROOM_HEIGHT - 90)
...
rooms = {
    "starting_room": {
        "color": (55, 188, 31),
        "doors": [{"rect": pygame.Rect(WIDTH//2-40, 20, 80, 90), "target": "lobby", "spawn": (2, HEIGHT//2)}],
        "enemies": [],  # no enemies in starting room
    },
    "lobby": {
        "color": (32, 12, 36),
        "doors": [{"rect": pygame.Rect(WIDTH-110, HEIGHT//2-55, 80, 110), "target": "kitchen", "spawn": (WIDTH//2, HEIGHT//2)}],
    ...
    "library": {
        "color": (5, 40, 62),
        "doors": [{"rect": pygame.Rect(WIDTH//2-40, HEIGHT-100, 80, 80), "target": "kitchen", "spawn": (WIDTH-140, HEIGHT//2)}],
        "enemies": make_enemies(random.randint(1,3), speed=2),
    }
}
...
while running:
    ...
    if hp <= 0:
        ...
        over2 = ui.render("Enter: Restart | Esc: Quit", True, WHITE)
