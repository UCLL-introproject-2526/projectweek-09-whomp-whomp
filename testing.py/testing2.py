...
class Config:
    ...
    METAL_SPEAR_TOKEN_REQUIREMENT = 10
...
class Enemy:
    ...
    def update_position(self, player_pos: Tuple[int, int], walls: List[pygame.Rect]):
        """Update enemy position with wall collision and AI pathfinding"""
        ...
        vx = int(self.speed * dx / dist)
        vy = int(self.speed * dy / dist)
        ...
        self.frame = (self.frame + Config.ANIMATION_SPEED) % 9
...
class RoomGenerator:
    ...
    def generate_floor(floor_num: int) -> Dict[str, Room]:
        ...
        for idx, (row, col) in enumerate(room_positions):
            ...
            elif idx == 10 and floor_num < Config.TOTAL_FLOORS:
                room_type = RoomType.STAIRS_UP
            elif idx == 5 and floor_num > 1:
    ...
    def _create_enemies(count: int, speed: int, hp: int = Config.ENEMY_DEFAULT_HP) -> List[Enemy]:
        ...
        for _ in range(count):
            x = random.randint(Config.WALL_THICKNESS + 100, Config.ROOM_WIDTH - Config.WALL_THICKNESS - 100)
            y = random.randint(Config.WALL_THICKNESS + 100, Config.ROOM_HEIGHT - Config.WALL_THICKNESS - 100)
            enemies.append(Enemy(
                rect=pygame.Rect(x, y, Config.ENEMY_SIZE, Config.ENEMY_SIZE + 16),
                hp=hp,
                max_hp=hp,
                speed=speed,
...
class Game:
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("Hotel Transylvania - Frankenstein Mansion Edition")
        self.clock = pygame.time.Clock()

        # Fonts
        self.ui_font = pygame.font.SysFont('Arial', 32, bold=True)
    ...
    def load_sprites(self):
        """Load and prepare sprite sheets and backgrounds"""
        # Try to load backgrounds
        try:
            self.start_bg = pygame.image.load("img/Startscherm.jpg").convert()
            self.start_bg = pygame.transform.scale(self.start_bg, (Config.SCREEN_WIDTH, Config.SCREEN_HEIGHT))
        except:
            self.start_bg = None

        try:
            self.floor_bg = pygame.image.load("img/stone.jpg").convert()
            self.floor_bg = pygame.transform.scale(self.floor_bg, (Config.SCREEN_WIDTH, Config.SCREEN_HEIGHT))
        except:
            self.floor_bg = None
        ...
        try:
            ...
            self.player_sprites = self._load_spritesheet(
                ...
                {Direction.DOWN: 0, Direction.LEFT: 1, Direction.RIGHT: 2, Direction.UP: 3}
            )
        except:
            self.player_sprites = self._create_fallback_player_sprites()

        # Try to load enemy sprites
        try:
            enemy_sheet = pygame.image.load("img/skeleton.png").convert_alpha()
            self.enemy_sprites = self._load_spritesheet(
                enemy_sheet,
                Config.ENEMY_FRAME_WIDTH,
                Config.ENEMY_FRAME_HEIGHT,
                {Direction.UP: 8, Direction.LEFT: 9, Direction.DOWN: 10, Direction.RIGHT: 11}
            )
        except:
            self.enemy_sprites = self._create_fallback_enemy_sprites()

    def _load_spritesheet(self, sheet: pygame.Surface, frame_w: int, frame_h: int,
                          row_map: Dict[Direction, int]) -> Dict[Direction, List[pygame.Surface]]:
    ...
    def _create_fallback_player_sprites(self) -> Dict[Direction, List[pygame.Surface]]:
        """Create simple colored sprites as fallback for player"""
        sprites = {}
        colors = {
            Direction.DOWN: (100, 150, 200),
            Direction.LEFT: (150, 100, 200),
            Direction.RIGHT: (200, 150, 100),
            Direction.UP: (150, 200, 100)
        }
    ...
    def _create_fallback_enemy_sprites(self) -> Dict[Direction, List[pygame.Surface]]:
        """Create simple colored sprites as fallback for enemies"""
        sprites = {}
        for direction in Direction:
            sprites[direction] = []
            for _ in range(9):
                surf = pygame.Surface((Config.ENEMY_FRAME_WIDTH, Config.ENEMY_FRAME_HEIGHT), pygame.SRCALPHA)
                pygame.draw.rect(surf, Color.GREEN, (16, 16, 32, 32))
                sprites[direction].append(surf)
        return sprites
    ...
    def get_walls(self) -> List[pygame.Rect]:
        """Get wall rectangles"""
        return [
            pygame.Rect(0, 0, Config.ROOM_WIDTH, Config.WALL_THICKNESS),
            pygame.Rect(0, Config.ROOM_HEIGHT - Config.WALL_THICKNESS, Config.ROOM_WIDTH, Config.WALL_THICKNESS),
            pygame.Rect(0, 0, Config.WALL_THICKNESS, Config.ROOM_HEIGHT),
            pygame.Rect(Config.ROOM_WIDTH - Config.WALL_THICKNESS, 0, Config.WALL_THICKNESS, Config.ROOM_HEIGHT)
        ]
    ...
    def handle_input(self, keys: pygame.key.ScancodeWrapper):
        ...
        self.player_frame = (self.player_frame + Config.ANIMATION_SPEED) if moving else int(self.player_frame)

    def get_attack_rect(self) -> pygame.Rect:
        """Get attack hitbox based on player direction"""
        r = self.player_rect
        rng = Config.ATTACK_RANGE
    ...
    def try_attack(self):
        ...
        draw_rect = attack_box.copy()
        draw_rect.x -= cam_x
        draw_rect.y -= cam_y
        pygame.draw.rect(self.screen, (255, 220, 120), draw_rect)

        for enemy in self.current_room.enemies:
            if enemy.is_alive and attack_box.colliderect(enemy.rect):
                if enemy.take_damage(self.current_damage):
                    self.tokens += 1
                    self.show_popup("+1 Token (monster defeated)", 1500)
    ...
    def check_collisions(self, keys: pygame.key.ScancodeWrapper):
        ...
        for door in self.current_room.doors:
            if self.player_rect.colliderect(door.rect):
                ...
                hint = self.ui_font.render(f"E: enter {target_room.name}", True, Color.WHITE)
                self.screen.blit(hint, (16, Config.SCREEN_HEIGHT - 36))
    ...
    def update(self):
        ...
        for enemy in self.current_room.enemies:
            if enemy.is_alive:
                enemy.update_position((self.player_rect.centerx, self.player_rect.centery), walls)
    ...
    def draw(self):
        ...
        if self.floor_bg:
            self.screen.blit(self.floor_bg, (-cam_x, -cam_y))
        ...
        for wall in self.get_walls():
            ...
            draw_rect.x -= cam_x
            draw_rect.y -= cam_y
            pygame.draw.rect(self.screen, Color.WALL, draw_rect)
        ...
        for door in self.current_room.doors:
            ...
            # Door name tag
            target_room = self.all_rooms[door.target_room]
            tag = self.ui_font.render(target_room.name, True, Color.WHITE)
            self.screen.blit(tag, (draw_rect.centerx - tag.get_width()//2, draw_rect.top - 24))
        ...
        for enemy in self.current_room.enemies:
            if enemy.is_alive:
                # Enemy sprite
                frame_idx = int(enemy.frame) % len(self.enemy_sprites[enemy.direction])
                sprite = self.enemy_sprites[enemy.direction][frame_idx]
                sprite_rect = enemy.rect.copy()
                sprite_rect.x -= cam_x + (sprite.get_width() - enemy.rect.width) // 2
                sprite_rect.y -= cam_y + (sprite.get_height() - enemy.rect.height) // 2
                ...
                health_bg = pygame.Rect(draw_rect.x, draw_rect.y - 10, draw_rect.width, 6)
                health_rect = pygame.Rect(draw_rect.x, draw_rect.y - 10, health_width, 6)
                pygame.draw.rect(self.screen, Color.RED, health_bg)
                pygame.draw.rect(self.screen, Color.GREEN, health_rect)

        # Draw player
        frame_idx = int(self.player_frame) % len(self.player_sprites[self.player_direction])
        player_sprite = self.player_sprites[self.player_direction][frame_idx]
        sprite_rect = self.player_rect.copy()
        sprite_rect.x -= cam_x + (player_sprite.get_width() - self.player_rect.width) // 2
        sprite_rect.y -= cam_y + (player_sprite.get_height() - self.player_rect.height) // 2
    ...
    def draw_hud(self):
        ...
        for i in range(Config.PLAYER_MAX_HP):
            color = (220, 60, 60) if i < self.player_hp else Color.DARK_HEART
            x, y = 16 + i * 22, 18
            pygame.draw.circle(self.screen, color, (x, y), 8)
            pygame.draw.circle(self.screen, color, (x + 10, y), 8)
            pygame.draw.polygon(self.screen, color, [(x-5, y+4), (x+15, y+4), (x+5, y+20)])

        # Stats
        weapon = "Metal spear" if self.has_metal_spear else "Wooden spear"
        info = f"Tokens: {self.tokens} | Weapon: {weapon} | Floor {self.current_floor}"
        info_surf = self.ui_font.render(info, True, Color.WHITE)
        self.screen.blit(info_surf, (16, 50))
        ...
        # Map hint
        map_hint = self.tiny_font.render("Press M for Map | F3 for Debug", True, Color.WHITE)
        self.screen.blit(map_hint, (Config.SCREEN_WIDTH - 220, 20))
        ...
        if self.popup_message and pygame.time.get_ticks() < self.popup_until:
            ...
            self.screen.blit(popup_surf, (16, 115))
        ...
        if self.debug_mode:
            ...
            debug_lines = [
                ...
                f"Player pos: {self.player_rect.x}, {self.player_rect.y}",
                f"Camera: {cam_x}, {cam_y}"
            ]
            for i, line in enumerate(debug_lines):
                text = self.small_font.render(line, True, Color.YELLOW)
                self.screen.blit(text, (10, Config.SCREEN_HEIGHT - 70 + i * 20))
    ...
    def show_start_screen(self):
        ...
        # Title
        title = self.title_font.render("Frankenstein Mansion", True, Color.WHITE)
        self.screen.blit(title, title.get_rect(center=(Config.SCREEN_WIDTH//2, 80)))

        subtitle = self.ui_font.render("Hotel Transylvania Edition", True, Color.YELLOW)
        self.screen.blit(subtitle, subtitle.get_rect(center=(Config.SCREEN_WIDTH//2, 140)))

        # Instructions
        instructions = [
            "4 Floors | 80 Rooms | Survive the Night",
            "",
        ...
        for line in instructions:
            ...
            if "Press ENTER" in line:
                ...
                color = Color.WHITE
                font = self.ui_font if y < 220 else self.small_font

            text = font.render(line, True, color)
            self.screen.blit(text, text.get_rect(center=(Config.SCREEN_WIDTH//2, y)))
            y += 40 if font == self.ui_font else 30
    ...
    def show_name_input_screen(self):
        ...
        prompt = self.ui_font.render(f"Voer je naam in: {self.player_name}", True, Color.WHITE)
        self.screen.blit(prompt, prompt.get_rect(center=(Config.SCREEN_WIDTH//2, Config.SCREEN_HEIGHT//2 + 200)))
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.button == 1:
                        ...
                        elif not self.game_over and not self.victory and not self.show_start_screen_flag and not self.show_name_input:
                            self.try_attack()
